...
# 運用・メンテナンス
- BigQuery の費用が改定された場合「[3. 料金単価テーブルの作成](#3-料金単価テーブルの作成)」で作成した料金単価テーブルを更新し、ダッシュボードで適切な費用が表示されるよう更新を行います。
- 料金単価テーブルの「application_start_date」が最新のレコードが、利用料金の計算時に反映されます。
## 料金単価テーブルの更新手順
1. [3. 料金単価テーブルの作成](#3-料金単価テーブルの作成)を参照し、最新の料金単価を確認
2. 以下のSQLを書き換えてから実行
```sql
-- コンピュート料金単価
-- xxxxの部分を料金単価に、yyyy-mm-ddの部分を確認した日付に書き換える
